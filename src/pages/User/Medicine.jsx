...
import { collection, firestore, onSnapshot, query } from '../../fb';
...
function Medicine({ userId }) {
  ...
  useEffect(() => {
    if (userId) {
      onSnapshot(
        query(collection(firestore, 'users', userId, 'medicines')),
        (snap) => {
          setMedicines(
            snap.docs.map((doc) => ({
              ...doc.data(),
  ...
  const renderSection = (title, filterCondition) => {
    const filteredMedicines = medicines.filter((i) =>
      i.instruction.includes(filterCondition)
    );
